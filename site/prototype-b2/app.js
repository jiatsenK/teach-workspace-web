...
function constructionBranches(mode, coursePayload, hierarchyCourse) {
  ...
  return `<div class="b2-branches"><p>目前 snapshot 是同一門課；B2 先分成兩個入口讓你判斷。</p><button type="button" data-proto-mode="exam" class="${mode === 'exam' ? 'is-active' : ''}"><strong>考古題演練</strong><span>${examCount} 個學習回合</span></button><button type="button" data-proto-mode="topic" class="${mode === 'topic' ? 'is-active' : ''}"><strong>主題學習</strong><span>${topicCount} 個主題</span></button></div>`;
}
...
function learningView(data) {
  ...
  return `<main class="b2-learning-shell ${closed ? 'is-nav-closed' : ''}">${rail({ ...data, selectedLessonId, mode, closed })}<section class="b2-reader"><div class="b2-reader-tabs"><button type="button" data-proto-nav-jump>開啟目錄</button><button type="button" data-proto-section="learn" class="${section === 'learn' ? 'is-active' : ''}">讀課文</button><button type="button" data-proto-section="history" class="${section === 'history' ? 'is-active' : ''}">看學習足跡</button><small>${section === 'history' ? '查看這門課最近學過什麼' : '一次只看一課'}</small></div><div class="b2-reader__scroll">${content}</div></section></main>`;
}
...
(async () => {
  document.getElementById('app').innerHTML = '<div style="padding:2rem">正在讀取 Learning Studio Preview…</div>';
  try {
